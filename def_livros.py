...
class AppBD:
    ...
    def selecionarDados(self):
        try:
            ...
            sql_select_query = """select * from "livros" """
    ...
    # Inserir Produto
    # -----------------------------------------------------------------------------
    def inserirDados(self, codigo, data, titulo, autor, edicao):
        try:
            self.abrirConexao()
            cursor = self.connection.cursor()
            postgres_insert_query = """ INSERT INTO "livros" 
          ("codigo", "data", "titulo", "autor", "edicao") VALUES (%s, %s, %s, %s, %s)"""
            record_to_insert = (codigo, data, titulo, autor, edicao)
    ...
    # Atualizar Produto
    # -----------------------------------------------------------------------------
    def atualizarDados(self, codigo, data, titulo, autor, edicao):
        try:
            self.abrirConexao()
            cursor = self.connection.cursor()
            ...
            print(record)
            # Atualizar registro
            sql_update_query = """Update "livros" set "titulo" = %s, "data" = %s, 
            "autor" = %s, "edicao" = %s where "codigo" = %s"""
            cursor.execute(sql_update_query, (titulo, data, autor, edicao, codigo))
            self.connection.commit()
            count = cursor.rowcount
    ...
    def excluirDados(self, codigo):
        try:
            ...
            cursor = self.connection.cursor()
            # Atualizar registro
            sql_delete_query = """Delete from "livros" 
            where "codigo" = %s"""
    ...
    def buscarDados(self, codigo):
        try:
            self.abrirConexao()
            cursor = self.connection.cursor()
            sql_select_query = """select * from "livros" where "codigo" = %s"""
            cursor.execute(sql_select_query, (codigo,))
            record = cursor.fetchone()
            return record
